...
bool CGUIDialogAccessPoints::OnAction(const CAction &action)
{
  if (action.GetID() == ACTION_SELECT_ITEM)
  {
    ...
    if (!m_doing_connection)
    {
      ...
      if (connections[iItem]->GetState() == NETWORK_CONNECTION_STATE_CONNECTED)
      {
         if (CGUIDialogYesNo::ShowAndGetInput(33204, 33205, -1, -1))
         {
           connections[iItem]->Disconnect();
           g_application.getNetwork().GetPassphraseStorage()->InvalidatePassphrase(connections[iItem]->GetUUID(), connections[iItem].get());
         }
         return true;
      }
