...
bool CNetworkManager::ReceiveNetworkEvent(const NetworkEventDataPtr data)
{
  if (!m_instance)
    return false;
  return m_instance->ReceiveNetworkEvent(this, data);
}

IPassphraseStorage* CNetworkManager::GetPassphraseStorage()
{
  if (!m_instance)
    return false;

  return m_instance->GetPassphraseStorage();
}
...
void CNetworkManager::OnConnectionStateChange(ConnectionState state)
{
  ...
  CGUIMessage msg(GUI_MSG_UPDATE, WINDOW_SETTINGS_SYSTEM, g_windowManager.GetActiveWindow());
...
void CNetworkManager::OnConnectionListChange(ConnectionList list)
{
  ...
  {
    m_defaultConnection = CConnectionPtr(new CNullConnection());
    OnConnectionStateChange(NETWORK_CONNECTION_STATE_DISCONNECTED);
  }
