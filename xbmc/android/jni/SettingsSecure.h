...
class CJNISettingsSecure : public CJNIBase
{
public:
  CJNISettingsSecure(jni::jhobject const& object) : CJNIBase(object) {};
  ~CJNISettingsSecure() {};
...
//  static bool putConfiguration(const CJNIContentResolver&, android.content.res.Configuration);
  static bool isLocationProviderEnabled(const CJNIContentResolver& resolver, const std::string &provider);
  static void setLocationProviderEnabled(const CJNIContentResolver& resolver, const std::string &provider, bool enabled);
